...
export default function MatchingScreen() {
      ...
      <View className="flex-1 bg-dark-900 p-4">
        <View className="mb-6">
          ...
          <Text className="text-dark-300 text-center mb-8">
            Chọn cấp độ để bắt đầu luyện tập ghép đôi
          </Text>
        </View>
        <FlatList
          data={[1, 2, 3, 4, 5, 6]}
  ...
  const words = getVocabByLevel(selectedLevel).slice(0, 10); // Limit to 10 words per session

  return <MatchingPractice words={words} onComplete={handleComplete} />;
}
